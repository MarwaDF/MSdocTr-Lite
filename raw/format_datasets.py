...
def format_IAM_line():
    ...
    """
    source_folder = "Datasets/raw/IAM"
    target_folder = "Datasets/formatted/IAM_lines"
    tar_filename = "Datasets/raw/IAM/lines.tgz"
    ...
    make_files(gt,target_folder)
    with open(os.path.join(target_folder, "chars.txt"), "w") as f:
        f.write(str(sorted(list(charset))))
...
def format_IAM_paragraph():
    ...
    """
    source_folder = "Datasets/raw/IAM"
    target_folder = "Datasets/formatted/IAM_paragraph"
    ...
    for set_name in ["train", "valid", "test"]:
        ...
        for page in xml_root:
            ...
            for section in page:
                ...
                gt[set_name][new_name] = paragraph["text"]
               # print(gt)
...
def format_RIMES_line():
    ...
    """
    source_folder = "Datasets/raw/RIMES"
...
def format_RIMES_paragraph():
    ...
    for set_name, xml_path in zip(["train", "eval"], [train_xml_path, eval_xml_path]):
        ...
        for page in xml_root:
            ...
            charset = charset.union(set(full_text))
            gt[new_set_name][new_name] =paragraph["text"]
             

    # with open(os.path.join(target_folder, "labels.pkl"), "w") as f:
    #      pickle.dump({
    #          "ground_truth": gt,
...
def format_READ2016_block():
    ...
    """
    source_folder = "Datasets/raw/READ_2016"
    target_folder = "formatted/READ2016_blocks"
    ...
    charset = set()
    for set_name in ["train", "valid", "test"]:
        with open(os.path.join(target_folder, set_name+".txt"), "w") as f:
                    ...
                    for text_region in xml_root[1][1:]:
                        ...
                        for balise in text_region:
                            ...
                            if(counter == 4 ):
                                ...
                                gt[set_name][new_img_name] = line_label #{"text": line_label, }
                                f.write(new_img_path + " " + line_label+"\n")
...
def make_files(gt,folder):
    for set_name in ["train", "valid", "test"]:
        with open(os.path.join(folder, set_name+".txt"), "w") as f:
            for key in gt[set_name].keys():
                f.write(key + " " + str(gt[set_name][key])+"\n")
...
def format_READ2016_paragraph():
    ...
    """
    source_folder = "/home/marwa/VerticalAttentionOCR/Datasets/raw/READ_2016"
    target_folder = "/home/marwa/VerticalAttentionOCR/Datasets/formatted/READ_2016_paragraph"
    if os.path.isdir(target_folder):
        shutil.rmtree(target_folder)
    os.makedirs(target_folder)
        ...
        with open(os.path.join(target_folder, set_name+".txt"), "w") as f:
            ...
            for xml_file_name in os.listdir(xml_fold_path):
                ...
                for text_region in xml_root[1][1:]:
                    ...
                    curr_img = img[top:bottom + 1, left:right + 1]
                    Image.fromarray(curr_img).save(new_img_path)
                    gt[set_name][new_img_name] = pg_label#{"text": pg_label, }
                    f.write(new_img_path + " " + pg_label.replace("\n","|")+"\n")
